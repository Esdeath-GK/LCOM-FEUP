...
void (kbc_ih_proj)(uint8_t *key_scanned) {
	uint8_t key_sys_inb;
	uint8_t check_status;
	while (1) {
		util_sys_inb(STAT_REG, &check_status);

		if (check_status & OUT_FULL) {
			util_sys_inb(OUT_BUF, &key_sys_inb);
			*key_scanned = key_sys_inb;
			// printf("Read %d\n", data_one);
			// Check for errors
			if ((check_status & (ERROR_CHECK)) == 0){
				return;
			}
			else{
				return;
...
int getbyteslist(int position){
	if(position > 2 || position < 0){
		return -1;
	}
	return bytes_list[position];
}
